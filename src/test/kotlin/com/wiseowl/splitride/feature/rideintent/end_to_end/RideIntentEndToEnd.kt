...
class RideIntentEndToEnd(@Autowired val restTestClient: RestTestClient) {
    ...
    private val createRideIntentRequestBody = CreateRideIntentRequestDTO(
        ...
        scheduleType = ScheduleType.Future("2027-02-28T22:14:45.000Z"),
        flexibleMinutes = 10
    )
    ...
    fun createRideIntent() {
        val request = CreateRideIntentRequestDTO(
            ...
            destinationLat = 28.12441,
            destinationLng = 78.124413,
            scheduleType = ScheduleType.Future(Instant.now().plusSeconds(1000*60*15).toString()),
            flexibleMinutes = 10
        )

        restTestClient.post().uri("/api/ride-intents/create")
        ...
        restTestClient.post().uri("/api/ride-intents/create")
            ...
            .body(
                createRideIntentRequestBody
                    .copy(
                        scheduleType = ScheduleType.Future(Instant.now().minusSeconds(60 * 15).toString()),
                    )
            )
            .exchangeAuthorized()
...
//        val createdRideIntent = restTestClient.post().uri("/api/ride-intents/create")
//            .contentType(MediaType.APPLICATION_JSON)
...
//    }
    ...
    fun groupHasSpaceForOne_OneRideIntentJoins_GroupBecomesFull() {
        val createdRideIntent1 = restTestClient.post().uri("/api/ride-intents/create")
            .contentType(MediaType.APPLICATION_JSON)
            .body(createRideIntentRequestBody)
            .exchangeAuthorized()
            .expectBody<SplitRideResponse<RideIntentResponseDTO>>()
        ...
        val createdRideIntent2 = restTestClient.post().uri("/api/ride-intents/create")
            .contentType(MediaType.APPLICATION_JSON)
            .body(createRideIntentRequestBody)
            .exchangeAuthorized()
            .expectBody<SplitRideResponse<RideIntentResponseDTO>>()
        ...
        val createdRideIntent3 = restTestClient.post().uri("/api/ride-intents/create")
            .contentType(MediaType.APPLICATION_JSON)
            .body(createRideIntentRequestBody)
            .exchangeAuthorized()
            .expectBody<SplitRideResponse<RideIntentResponseDTO>>()
        ...
        restTestClient.get().uri("/api/ride-intents/search/schedule?rideIntentId=${createdRideIntent1?.id}")
            .exchangeAuthorized()

        restTestClient.get().uri("/api/ride-intents/search/schedule?rideIntentId=${createdRideIntent2?.id}")
            .exchangeAuthorized()

        val rideSearchProcessId = restTestClient.get().uri("/api/ride-intents/search/schedule?rideIntentId=${createdRideIntent3?.id}")
            .exchangeAuthorized()
            .expectBody< SplitRideResponse<ScheduleSearchResponse>>()
            .returnResult().responseBody?.data?.searchProcessId

        assertThat(rideSearchProcessId).isNotNull()
    }
...
//            scheduleType = ScheduleType.Future(Instant.now().plusSeconds(10).toString())
//        )
//        val createdRideIntent1 = restTestClient.post().uri("/api/ride-intents/create")
//            .contentType(MediaType.APPLICATION_JSON)
//            .body(rideIntentBody)
