...
class RideSearchProcessingJob(
    ...
    fun scheduleSearch() {
        ...
        idleRideSearchProcesses.forEach { process ->
            val rideIntent = rideIntentRepository.findById(process.rideIntentId).getOrNull()
            if (rideIntent == null) {
                ...
                val foundMatchingGroup = openGroups.firstOrNull { group ->
                    val isSourceWithinBound = geoCalculator.distanceInKm(
                        rideIntent.sourceLat,
                        rideIntent.sourceLng,
                        group.sourceLat,
                        group.sourceLng
                    ) < MATCH_BOUND_DISTANCE_KM
                    val isDestinationWithinBound = geoCalculator.distanceInKm(
                        rideIntent.destinationLat,
                        rideIntent.destinationLng,
                ...
                if (foundMatchingGroup != null) {
                    ...
                    rideSearchProcessRepository.save(
                        process.copy(
                            status = RideSearchProcessState.Grouped(
                                foundMatchingGroup.id
                            )
                        )
                    )
                } else {
                    val searchingRideSearchProcesses = rideSearchProcessRepository.findAllByStatusInAndIdNotLike(
                        listOf(RideSearchProcessState.Searching, RideSearchProcessState.Idle),
                        process.id!!
                    )
                    val matchingRideSearchProcesses = searchingRideSearchProcesses.firstOrNull { process ->
                        ...
                        val isSourceWithinBound = geoCalculator.distanceInKm(
                            rideIntent.sourceLat,
                            rideIntent.sourceLng,
                            matchedRideIntent.sourceLat,
                            matchedRideIntent.sourceLng
                        ) < MATCH_BOUND_DISTANCE_KM
                        val isDestinationWithinBound = geoCalculator.distanceInKm(
                            rideIntent.destinationLat,
                            rideIntent.destinationLng,
                            matchedRideIntent.destinationLat,
                            matchedRideIntent.destinationLng
                        ) < MATCH_BOUND_DISTANCE_KM
                        val isWithingTimeBound = when {
                            rideIntent.scheduleType is ScheduleType.Future && matchedRideIntent.scheduleType is ScheduleType.Future -> {
                                ...
                                val doesTimeOverlaps =
                                    rideIntentStartTime.isAfter(matchedRideIntentStartTime) && rideIntentStartTime.isBefore(
                                        matchedRideIntentStartTime.plusSeconds(matchedRideIntent.flexibleMinutes * 60L)
                                    )
                                doesTimeOverlaps
                            }
                    ...
                    if(matchingRideSearchProcesses!=null){
                        ...
                        rideSearchProcessRepository.save(process.copy(status = RideSearchProcessState.Idle))
                    }
                }
            }
        }
